...
<div class="row g-3">
    ...
    <div class="col-md-3">
        ...
        <select name="level" class="form-select" required>
            @foreach(['basico'=>'Básico','intermedio'=>'Intermedio','avanzado'=>'Avanzado'] as $v=>$lbl)
                <option value="{{ $v }}" @selected(old('level', $course->level ?? '')==$v)>{{ $lbl }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-3 d-flex align-items-end">
        <div class="form-check form-switch">
            <input type="checkbox" name="is_published" value="1" class="form-check-input" id="is_published" @checked(old('is_published', $course->is_published ?? false))>
